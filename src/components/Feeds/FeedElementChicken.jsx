...
const FeedElementChicken = () => {
	...
	if (elementSelected) {
		...
		packageVars.description = "Филе из цыплят с трюфелями в бульоне."
		packageVars.link = null
	}
	if (elementDisabled) {
		FeedElementClass = ['feeds__element', 'element-feeds']
		FeedElementClass.push('disabled')
		packageVars.description = "Печалька, с курой закончился."
		packageVars.link = null
	}
	return (
		<div className={FeedElementClass.join(' ')}>
			...
			<div className="element-feeds__description description">
				<span>
					{packageVars.description}
					<button className="element-feeds__button"
						onClick={(e) => {
							e.preventDefault();
...
export default FeedElementChicken